...
#Taking out variables I don't need
...
#Can also use gather() for tidy data  
```

#Exploring the data:
###Total species detected in different fragments 
```{r}
fragments_plot <- ggplot(data=surveys_no_birds,
mapping= aes(x=fragment, y=species, fill=species))+
geom_bar(stat="identity", alpha=0.8)
plot(fragments_plot)

```
The abrove graph shows abundance and richness? of species for each fragment plot. The colors represent individual species and the bar height represents total species abundance. We can see that fragment 87 has the greatest total species abundance. Plot 89 is the least rich with only two different species detected. Conversely, plot 12 has the greatess species richness, with a total of 9 different species detected.
...
SCOPE_plot <- ggplot(data= surveys_no_birds,
  ...
  geom_bar(stat="identity", alpha=0.8)
plot(SCOPE_plot)
#Not sure why my bars are so skinny here
```
...
#q+ theme(axis.text.x=element_text(angle = 90, hjust = 1)) (I think this is how youcan manipulate the x axis label)

```

boxplot(TEN~species, data=surveys_no_birds, xaxt="n")# no x axis
